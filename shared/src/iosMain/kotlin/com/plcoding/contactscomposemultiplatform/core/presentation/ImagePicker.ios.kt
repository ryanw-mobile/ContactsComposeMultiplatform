...
actual class ImagePicker(
    ...
    private val delegate = object : NSObject(), UIImagePickerControllerDelegateProtocol, UINavigationControllerDelegateProtocol {
        override fun imagePickerController(
            ...
            val imageNsData = UIImageJPEGRepresentation(image = didFinishPickingImage, compressionQuality = 1.0) ?: return
            val bytes = ByteArray(imageNsData.length.toInt())
            memcpy(bytes.refTo(index = 0), imageNsData.bytes, imageNsData.length)
    ...
    actual fun pickImage() {
        rootController.presentViewController(imagePickerController, animated = true) {
