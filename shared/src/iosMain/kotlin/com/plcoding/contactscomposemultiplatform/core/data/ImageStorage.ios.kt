...
actual class ImageStorage {
    ...
    actual suspend fun saveImage(bytes: ByteArray): String {
        // We don't have IO Dispatcher on IOS
        ...
        return withContext(Dispatchers.Default) {
            memScoped {
                NSData.dataWithContentsOfFile(fileName)?.let { bytes ->
                    ...
                    bytes.getBytes(array.refTo(index = 0).getPointer(scope = this), bytes.length)
    ...
    actual suspend fun deleteImage(fileName: String) {
        withContext(Dispatchers.Default) {
            fileManager.removeItemAtPath(path = fileName, error = null)
        }
    }
}
